...
/// 保活当前关着。
///
/// 它从来没被验证有效过 —— 而且直到今天之前它根本没真正发出去过(特权连接
/// 成功后没建自己的 GATT 客户端,keepAlive 一直在空转)。刚开始真发,掉线就
/// 密了起来:六分钟四次,此前约十一分钟一次;其中一次掉线的同一毫秒还有一条
/// 保活读取失败。数据是混淆的,不能断定因果,但它往手柄输入所用的同一条链路
/// 上定期塞数据,嫌疑成立而收益未知,先关掉做对照。
///
/// 要重新试就把这里改回 true,机制原样还在。
pub const KEEPALIVE_ENABLED: bool = false;
